...
equation
  ...
  Documentation(info = "<html><head></head><body><div>This block solves an exponential-transendental equation of the form:</div><div><br></div><div><div>x = a + b e<sup>cx</sup></div><div><br></div><div>where b and c must be nonzero. The solution is found via the Lambert W functions:</div><div><br></div><div>x = a - W(-bc e<sup>ac</sup>) / c&nbsp;</div><div><br></div><div>where W(y) is a solution of: W(y) e<sup>W(y)</sup> = y .</div><div><br></div><div>If the argument (y = -bc e<sup>ac</sup>) of the W function is real and between -1/e and 0, then there are two real W values: W<sub>0</sub>&nbsp;(&gt; -1) and W<sub>-1</sub>&nbsp;(&lt; -1).</div><div>If the argument is real and &gt;=0, then there is one real value: W<sub>0</sub>.</div></div><div><br></div><div>The boolean outputs bx[1] and bx[2] denote whether real solutions for x exist. If their values are \"true\", then the corresponding x values contain the solutions: x[1] from W<sub>0</sub> and x[2] from W<sub>-1</sub>.</div></body></html>"));
